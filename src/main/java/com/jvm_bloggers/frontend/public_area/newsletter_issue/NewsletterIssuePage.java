...
public class NewsletterIssuePage extends AbstractFrontendPage {
    ...
    @SpringBean
    private NewsletterIssuePageRequestHandler requestHandler;

    public NewsletterIssuePage(PageParameters parameters) {
        super(parameters);
        long issueNumber = parameters.get(0).toLong(-1);
        Option<PublishedNewsletterIssue> foundIssue = requestHandler.findByIssueNumber(issueNumber);
        if (foundIssue.isDefined()) {
            add(new NewsletterIssuePanel(ISSUE_PANEL_ID, foundIssue.get()));
        } else {
            add(new Label(ISSUE_PANEL_ID, "Nie znaleziono wydania nr " + issueNumber));
        }
    }
